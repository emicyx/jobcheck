...
扩展 alarms 每 3 小时遍历已连接站点（原每小时，2026-09-03 降频控风控与解析成本），静默开隐藏标签页刷新投递页（站点自己的 JS 带登录态
...
### 收尾
- e2e 重写：`frontend/public/jc-e2e.html` 驱动「访问→自动上报→卡片」（面向真实站；
  mock 门户已于 v0.6.2 删除，飞书契约回归改用 golden 固化样本）
...
| 状态时效 | 默认每 3 小时后台同步（原每小时，2026-09-03 降频），浏览器在线即生效 |
